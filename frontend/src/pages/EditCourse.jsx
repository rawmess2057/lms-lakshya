...
const getBackendUrl = () => {
  ...
  // Production fallback: use same origin for API
...
const EditCourse = () => {
  ...
  useEffect(() => {
    if (formData.category) {
      ...
      // If current subject is not in the filtered list (and we are not loading initial data), reset it.
      // But we need to be careful not to reset it on initial load.
      // Initial load sets formData, then this runs. 
      // If formData.subject is set, we should check if it's valid for the new category.
      // However, for simplicity and to avoid bugs on load, we trust the DB data on load.
      // But if user changes category, handleInputChange handles the reset.
    } else {
      setFilteredSubjects([]);
    }
  ...
  const fetchCourse = async () => {
    try {
      ...
      setFormData({
        ...
        category: (courseData.category?._id) || (typeof courseData.category === 'string' && courseData.category.match(/^[0-9a-fA-F]{24}$/) ? courseData.category : '') || '',
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex items-center space-x-2 px-4 py-3 font-medium text-sm transition-colors ${activeTab === tab.id
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-gray-200 dark:border-gray-700">
                        ...
                        <th className="text-left py-3 px-4 font-semibold text-gray-900 dark:text-white">Videos</th>
                        <th className="text-left py-3 px-4 font-semibold text-gray-900 dark:text-white">Quiz Scores</th>
                        <th className="text-left py-3 px-4 font-semibold text-gray-900 dark:text-white">Assignments</th>
                        <th className="text-left py-3 px-4 font-semibold text-gray-900 dark:text-white">Live Sessions</th>
                            ...
                            >
                              ...
                              <td className="py-4 px-4">
                                <div className="flex items-center space-x-2">
                                  <div className="flex-1 bg-gray-200 dark:bg-gray-700 rounded-full h-2 max-w-[120px]">
                                    <div
                                      className={`h-2 rounded-full transition-all ${(student.progress?.completionPercentage || 0) === 100
                                        ? 'bg-green-600'
                              ...
                              <td className="py-4 px-4 text-sm text-gray-600 dark:text-gray-400">
                                <div className="flex flex-col">
                                  <span>{student.progress?.completedVideos || 0} / {student.progress?.totalVideos || 0}</span>
                              ...
                              <td className="py-4 px-4">
                                {student.quizStats ? (
                                  <div className="flex flex-col text-sm">
                                    ...
                                    {student.quizStats.completed > 0 && (
                                      <span className="text-xs text-gray-500">
                                        Avg: {student.quizStats.averageScore.toFixed(1)}%
                                      </span>
                                    )}
                                    {student.quizStats.passed > 0 && (
                                      <span className="text-xs text-green-600">
                                        Passed: {student.quizStats.passed}
                                      </span>
                                    )}
                                  </div>
                                ) : (
                              ...
                              <td className="py-4 px-4">
                                {student.assignmentStats ? (
                                  <div className="flex flex-col text-sm">
                                    ...
                                    {student.assignmentStats.completed > 0 && (
                                      <span className="text-xs text-gray-500">
                                        Avg: {student.assignmentStats.averageScore.toFixed(1)}%
                                      </span>
                                    )}
                                    {student.assignmentStats.graded > 0 && (
                                      <span className="text-xs text-blue-600">
                                        Graded: {student.assignmentStats.graded}
                                      </span>
                                    )}
                                  </div>
                                ) : (
                              ...
                              <td className="py-4 px-4">
                                {student.liveSessionAttendance ? (
                                  <div className="flex flex-col text-sm">
                                    <span className="font-medium text-gray-900 dark:text-white">
                                      {student.liveSessionAttendance.attended} / {student.liveSessionAttendance.total}
                                    </span>
                              ...
                              <tr className="bg-gray-50 dark:bg-gray-800/50">
                                <td colSpan="10" className="py-6 px-4">
                                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    {/* Quiz Scores - Detailed View */}
                                    <div>
                                      <h4 className="font-semibold text-gray-900 dark:text-white mb-3 flex items-center">
                                        <FiFileText className="mr-2" />
                                        Quiz Scores & Marks
                                      </h4>
                                      {student.quizStats && student.quizStats.scores && student.quizStats.scores.length > 0 ? (
                                        <div className="space-y-2">
                                          {student.quizStats.scores.map((quiz, idx) => (
                                            <div key={idx} className="p-3 bg-white dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-gray-600">
                                              ...
                                              <div className="text-xs text-gray-600 dark:text-gray-400">
                                                <div className="font-semibold text-primary-600 dark:text-primary-400">
                                                  Marks: {quiz.score} / {quiz.totalPoints}
                                                </div>
                                                <div>Percentage: {quiz.percentage.toFixed(1)}%</div>
                                                <div className={`mt-1 ${quiz.passed ? 'text-green-600' : 'text-red-600'}`}>
                                          ...
                                          <div className="mt-3 p-2 bg-primary-50 dark:bg-primary-900/20 rounded">
                                            <div className="text-xs text-gray-700 dark:text-gray-300">
                                              <div>Total Quizzes: {student.quizStats.total}</div>
                                              <div>Completed: {student.quizStats.completed}</div>
                                              <div className="font-semibold">Average Score: {student.quizStats.averageScore.toFixed(1)}%</div>
                                              <div className="text-green-600 dark:text-green-400">Passed: {student.quizStats.passed}</div>
                                            </div>
                                          </div>
                                        </div>
                                      ) : (
                                        <p className="text-sm text-gray-500 dark:text-gray-400">No quiz submissions yet</p>
                                      )}
                                    </div>

                                    {/* Assignment Scores - Detailed View */}
                                    <div>
                                      <h4 className="font-semibold text-gray-900 dark:text-white mb-3 flex items-center">
                                        <FiBook className="mr-2" />
                                        Assignment Scores
                                      </h4>
                                      {student.assignmentStats && student.assignmentStats.scores && student.assignmentStats.scores.length > 0 ? (
                                        <div className="space-y-2">
                                          {student.assignmentStats.scores.map((assignment, idx) => (
                                            <div key={idx} className="p-3 bg-white dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-gray-600">
                                              <div className="flex items-center justify-between mb-1">
                                                <span className="text-sm font-medium text-gray-900 dark:text-white truncate">
                                              ...
                                              <div className="text-xs text-gray-600 dark:text-gray-400">
                                                <div className="font-semibold text-primary-600 dark:text-primary-400">
                                                  Marks: {assignment.score} / {assignment.maxScore}
                                                </div>
                                                {assignment.maxScore > 0 && (
                                                  <div>Percentage: {((assignment.score / assignment.maxScore) * 100).toFixed(1)}%</div>
                                                )}
                                                <div className="mt-1">
                                                  Status: {assignment.graded ? 'Graded' : 'Pending Review'}
                                                </div>
                                              </div>
                                              <div className="text-xs text-gray-500 dark:text-gray-500 mt-1">
                                          ...
                                          <div className="mt-3 p-2 bg-primary-50 dark:bg-primary-900/20 rounded">
                                            <div className="text-xs text-gray-700 dark:text-gray-300">
                                              <div>Total Assignments: {student.assignmentStats.total}</div>
                                              <div>Completed: {student.assignmentStats.completed}</div>
                                              <div className="font-semibold">Average Score: {student.assignmentStats.averageScore.toFixed(1)}%</div>
                                              <div className="text-blue-600 dark:text-blue-400">Graded: {student.assignmentStats.graded}</div>
                                            </div>
                                          </div>
                                        </div>
                                      ) : (
                                    ...
                                    {/* Live Session Attendance - Detailed View */}
                                    <div>
                                      <h4 className="font-semibold text-gray-900 dark:text-white mb-3 flex items-center">
                                        <FiVideo className="mr-2" />
                                        Live Session Attendance
                                      </h4>
                                      {student.liveSessionAttendance && student.liveSessionAttendance.sessions && student.liveSessionAttendance.sessions.length > 0 ? (
                                        <div className="space-y-2">
                                          {student.liveSessionAttendance.sessions.map((session, idx) => (
                                            <div key={idx} className="p-3 bg-white dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-gray-600">
                                              <div className="flex items-center justify-between mb-1">
                                                <span className="text-sm font-medium text-gray-900 dark:text-white truncate">
                                                  {session.liveClassTitle}
                                                </span>
                                                <span className={`text-xs px-2 py-1 rounded ${session.status === 'present' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' :
                                                  session.status === 'late' ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200' :
                                              ...
                                              <div className="text-xs text-gray-600 dark:text-gray-400">
                                                ...
                                                <div>Joined: {new Date(session.joinedAt).toLocaleString()}</div>
                                                {session.leftAt && (
                                                  <div>Left: {new Date(session.leftAt).toLocaleString()}</div>
                                                )}
                                              </div>
                                              <div className="text-xs text-gray-500 dark:text-gray-500 mt-1">
                                          ...
                                          <div className="mt-3 p-2 bg-primary-50 dark:bg-primary-900/20 rounded">
                                            <div className="text-xs text-gray-700 dark:text-gray-300">
                                              <div>Total Sessions: {student.liveSessionAttendance.total}</div>
                                              <div className="font-semibold">Attended: {student.liveSessionAttendance.attended}</div>
                                            </div>
                                          </div>
                                        </div>
                                      ) : (
                                        <p className="text-sm text-gray-500 dark:text-gray-400">No live session attendance yet</p>
                                      )}
                                    </div>
                                  </div>
                                </td>
