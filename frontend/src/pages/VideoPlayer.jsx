...
const VideoPlayer = () => {
  ...
  const updateVideoProgress = async (progressPercent, completed) => {
    try {
      ...
      // Silently fail for video progress updates (429 errors are expected during heavy usage)
      // Progress will be saved on next successful update
      if (error.response?.status !== 429) {
        // Only log non-rate-limit errors in development
        if (process.env.NODE_ENV === 'development') {
          console.debug('Video progress update failed:', error.response?.status);
        }
      }
    }
  };
        ...
        <div className="lg:col-span-3">
          <div className="card mb-6">
            ...
            >
              ...
              {/* Conditional rendering: BunnyPlayer for Bunny videos, Drive iframe, or ReactPlayer */}
              {video.bunnyVideoId ? (
                (() => {
                  // Check if Bunny Stream is configured before using BunnyPlayer
                  const bunnyLibraryId = import.meta.env.VITE_BUNNY_STREAM_LIBRARY_ID;
                  if (!bunnyLibraryId) {
                    // Bunny Stream not configured - show helpful message
                    // Note: videoUrl is not available because backend removes it when bunnyVideoId exists
                    return (
                      <div className="bg-gray-900 rounded-lg flex items-center justify-center aspect-video">
                        <div className="text-center text-white p-6 max-w-md">
                          ...
                          <p className="text-sm text-gray-400">
                            Bunny Stream library ID is not configured. Please contact the administrator to set up the video streaming service.
                          </p>
                        </div>
                      </div>
                    );
                ...
                <div className="bg-gray-900 rounded-lg flex items-center justify-center aspect-video">
                  <div className="text-center text-white p-4">
                    <p className="text-red-400 mb-2">Video not available</p>
                    <p className="text-sm text-gray-400">No video URL or ID provided</p>
                  </div>
                </div>
              )}

              {/* Video Watermark Overlay - Only show for authenticated users */}
              {/* Note: Watermark may not work with iframe embeds, but keeping for non-Bunny videos */}
              {isAuthenticated && user && !video.bunnyVideoId && (
                <VideoWatermark
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {videos.map((v, index) => (
                <Link
                  ...
                  className={`flex items-center space-x-3 p-3 rounded-lg transition-colors ${v._id === videoId
