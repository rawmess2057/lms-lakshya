...
const ManageCourses = () => {
  ...
  // Filter subjects when category is selected
  useEffect(() => {
  ...
  const fetchCourses = async () => {
    try {
      ...
      // Don't show error if user is being redirected (401/403) or if it's a network error
      if (error.response?.status === 401 || error.response?.status === 403) {
        // User will be redirected by axios interceptor, don't show error
        return;
      }
      ...
      if (error.response?.status >= 500 || error.response?.data?.error) {
        toast.error(error.response?.data?.error || 'Failed to load courses. Please try again.');
            ...
            <div key={course._id} className="card">
              ...
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  ...
                  <span className={`px-2 py-1 text-xs rounded font-medium flex items-center space-x-1 ${course.isPublished && course.isActive
                ...
                {!course.isPublished || !course.isActive ? (
                  ...
                  <>
                    <button
                      ...
                      className={`flex-1 btn-outline text-sm min-w-[100px] ${course.isPublished ? 'text-yellow-600' : 'text-green-600'
                        }`}
            ...
            <form onSubmit={handleCreateCourse} className="p-6 space-y-4">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  Optional: Add a free demo/preview video URL (YouTube, Vimeo, or direct video link)
                </p>
              </div>
