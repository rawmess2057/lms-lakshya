...
// Helper to process thumbnail URLs (specifically for Google Drive)
const processThumbnailUrl = (url) => {
  ...
  if (driveMatch && driveMatch[1]) {
    ...
    // Convert to direct view URL for images
...
export const getCourses = asyncHandler(async (req, res) => {
  const { subject, teacher, level, search, page = 1, limit = 10, isPublished } = req.query;

  // Build query: Always filter by isActive=true, and isPublished based on user role or explicit parameter
  const query = { isActive: true };

  // If isPublished=true is explicitly requested, always filter by it (even for admins on public landing page)
  // Otherwise, admins see all courses, non-admins only see published courses
  if (isPublished === 'true') {
    query.isPublished = true;
  } else if (req.user && req.user.role === 'admin') {
    // Admin can see all courses (published and unpublished) when not explicitly filtering
    // Don't add isPublished filter for admin
  } else {
    // Non-admin users (including public/unauthenticated) only see published courses
    query.isPublished = true;
  }
...
export const createCourse = asyncHandler(async (req, res) => {
  ...
  // Validate Subject belongs to Category
  if (req.body.subject && req.body.category) {
    const subjectDoc = await Subject.findById(req.body.subject);
    if (subjectDoc && subjectDoc.category.toString() !== req.body.category.toString()) {
      return res.status(400).json({
        success: false,
        error: 'Selected subject does not belong to the selected category',
      });
    }
  }
...
export const updateCourse = asyncHandler(async (req, res) => {
  console.log(`Update Course Request: ID=${req.params.id}`);
  ...
  if (req.body.category) {
    // If it's an object (populated), get _id
    const catId = req.body.category._id || req.body.category;
    ...
    // Verify it exists in DB (optional but good)
    // const categoryExists = await mongoose.model('Category').findById(catId);
    // if (!categoryExists) return res.status(400).json({ success: false, error: 'Category not found' });
  }

  // Validate Subject if present
  if (req.body.subject) {
    const subId = req.body.subject._id || req.body.subject;
  ...
  // Validate Subject-Category consistency if both are present or if one is changing
  if (req.body.subject || req.body.category) {
    const targetSubjectId = req.body.subject || course.subject;
    const targetCategoryId = req.body.category || course.category;

    // Handle populated fields if they exist on course
    const subjId = targetSubjectId._id || targetSubjectId;
    const catId = targetCategoryId._id || targetCategoryId;

    if (subjId && catId) {
      // Use mongoose.model to avoid circular dependency issues if any, though import should work
      const subjectDoc = await Subject.findById(subjId);
      if (subjectDoc && subjectDoc.category && subjectDoc.category.toString() !== catId.toString()) {
        return res.status(400).json({
          success: false,
          error: 'Selected subject does not belong to the selected category',
        });
...
export const getCourseStudents = asyncHandler(async (req, res) => {
  ...
  const courseQuizzes = await Quiz.find({ course: course._id, isActive: true });
  const courseAssignments = await Assignment.find({ course: course._id, isActive: true });
  const courseLiveClasses = await LiveClass.find({ course: course._id });

  // Get progress for each student with quiz/assignment scores
  const studentsWithProgress = await Promise.all(
    enrolledStudents.map(async (student) => {
      ...
      const quizStats = {
        ...
        averageScore: 0,
        passed: 0,
        scores: quizSubmissions.map((sub) => ({
          quizId: sub.quiz?._id || sub.quiz,
          quizTitle: sub.quiz?.title || 'Unknown Quiz',
          score: sub.quizScore || 0,
          totalPoints: sub.quiz?.totalPoints || 0,
          percentage: sub.quizPercentage || 0,
          passed: sub.quizPassed || false,
          submittedAt: sub.submittedAt,
        })),
      };

      if (quizSubmissions.length > 0) {
        const totalPercentage = quizSubmissions.reduce((sum, sub) => sum + (sub.quizPercentage || 0), 0);
        quizStats.averageScore = totalPercentage / quizSubmissions.length;
        quizStats.passed = quizSubmissions.filter((sub) => sub.quizPassed).length;
      }

      // Calculate assignment statistics
      const assignmentStats = {
        total: courseAssignments.length,
        completed: assignmentSubmissions.length,
        averageScore: 0,
        graded: assignmentSubmissions.filter((sub) => sub.assignmentScore > 0 || sub.gradedBy).length,
        scores: assignmentSubmissions.map((sub) => ({
          assignmentId: sub.assignment?._id || sub.assignment,
          assignmentTitle: sub.assignment?.title || 'Unknown Assignment',
          score: sub.assignmentScore || 0,
          maxScore: sub.assignment?.maxScore || 0,
          graded: !!sub.gradedBy,
          submittedAt: sub.submittedAt,
        })),
      };
      ...
      if (assignmentSubmissions.length > 0) {
        const totalScore = assignmentSubmissions.reduce((sum, sub) => sum + (sub.assignmentScore || 0), 0);
        const totalMax = assignmentSubmissions.reduce((sum, sub) => sum + (sub.assignment?.maxScore || 0), 0);
        assignmentStats.averageScore = totalMax > 0 ? (totalScore / totalMax) * 100 : 0;
      }

      // Get live session attendance
      const liveSessionAttendance = [];
      for (const liveClass of courseLiveClasses) {
      ...
      return {
        ...
        progress: progress
          ? {
            ...
            totalVideos: progress.videosWatched?.length || 0,
            completedVideos: progress.videosWatched?.filter((v) => v.completed).length || 0,
          }
          : {
            completionPercentage: 0,
            lastAccessed: null,
        ...
        liveSessionAttendance: {
          total: courseLiveClasses.length,
          attended: liveSessionAttendance.length,
          sessions: liveSessionAttendance,
        },
      };
    })
  );
