...
/**
 * @desc    Create payment (DEPRECATED - Use manual payment instead)
 * @route   POST /api/payments
 * @access  Private/Student
 * @note    Mock payments have been removed. Please use /api/payments/manual endpoint.
 */
export const createPayment = asyncHandler(async (req, res) => {
  return res.status(400).json({
    success: false,
    error: 'Mock payments are no longer available. Please use manual payment methods (EasyPaisa, JazzCash, or Bank Transfer).',
  });
});
...
export const verifyPayment = asyncHandler(async (req, res) => {
  ...
  // Send payment approval email to student (non-blocking)
  sendPaymentApproval(
    payment.student,
    course.title,
    ...
    .then((result) => {
      if (result.success) {
        console.log(`✅ [PAYMENT] Payment approval email sent successfully to ${payment.student.email}`);
      } else {
        console.error(`❌ [PAYMENT] Failed to send payment approval email to ${payment.student.email}: ${result.error}`);
      }
    })
    .catch((error) => {
      console.error(`❌ [PAYMENT] Error sending payment approval email to ${payment.student.email}:`, error);
    });
...
export const submitManualPayment = asyncHandler(async (req, res) => {
  ...
  // Populate payment for email notification
  const populatedPayment = await Payment.findById(payment._id)
    .populate('student', 'name email')
    .populate('course', 'title');

  // Send admin notification for new payment request (non-blocking)
  // Only send if payment, student, and course are properly populated
  if (populatedPayment && populatedPayment.student && populatedPayment.course) {
    User.find({ role: 'admin', isActive: true })
      .select('email')
      .then((admins) => {
        if (admins.length === 0) {
          console.log('ℹ️  [PAYMENT] No admin users found to notify about new payment');
          return;
        }
        console.log(`📧 [PAYMENT] Notifying ${admins.length} admin(s) about new payment request`);
        
        admins.forEach((admin) => {
          sendAdminNewPaymentNotification(
            admin.email,
            populatedPayment,
            populatedPayment.student,
            populatedPayment.course
          )
            .then((result) => {
              if (result.success) {
                console.log(`✅ [PAYMENT] Admin payment notification sent successfully to ${admin.email}`);
              } else {
                console.error(`❌ [PAYMENT] Failed to send admin notification to ${admin.email}: ${result.error}`);
              }
            })
            .catch((error) => {
              console.error(`❌ [PAYMENT] Error sending admin notification to ${admin.email}:`, error);
            });
        });
      })
      .catch((error) => {
        console.error('❌ [PAYMENT] Error fetching admin emails:', error);
      });
  } else {
    console.warn('⚠️  [PAYMENT] Cannot send admin notification: payment, student, or course data is missing');
  }
