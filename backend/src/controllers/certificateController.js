...
import { sendCertificateCompletion } from '../utils/emailService.js';
...
export const generateCertificate = asyncHandler(async (req, res) => {
  ...
  // Send certificate completion email to student (non-blocking)
  sendCertificateCompletion(req.user, course.title, pdfResult.url)
    .then((result) => {
      if (result.success) {
        console.log(`✅ [CERTIFICATE] Certificate completion email sent successfully to ${req.user.email}`);
      } else {
        console.error(`❌ [CERTIFICATE] Failed to send certificate completion email to ${req.user.email}: ${result.error}`);
      }
    })
    .catch((error) => {
      console.error(`❌ [CERTIFICATE] Error sending certificate completion email to ${req.user.email}:`, error);
    });
